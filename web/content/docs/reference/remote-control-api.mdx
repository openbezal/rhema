...
The HTTP server listens on port `8080` (configurable) under the
`/api/v1/` prefix. Two endpoints are exposed.
...
Unknown commands and invalid JSON are rejected.
...
<Callout title="CORS is disabled by default" type="warn">
The HTTP server is built with Axum and does **not** advertise CORS
headers. Local-network and same-machine requests work fine; browser
fetches from a different origin will be blocked by the browser. Front
it with a reverse proxy if you need cross-origin access from a web
dashboard.
</Callout>
