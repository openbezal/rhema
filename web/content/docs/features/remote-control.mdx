...
By default, both protocols bind to `0.0.0.0` and HTTP runs **without
CORS** for local use only. Before exposing Rhema beyond the local
network, bind to `127.0.0.1` in Settings, or front it with a reverse
proxy that adds authentication and TLS. The repo's
