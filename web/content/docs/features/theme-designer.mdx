...
Rhema ships with a small set of opinionated starter themes that you can
clone and modify:

- **Lower-third** — a minimal bottom strip suitable for talking-head
  layouts.
- **Full-frame** — a centered overlay for cut-aways and IMAG.
- **Side-card** — an offset card that doesn't obscure the speaker's
  face.
...
- A Stream Deck or other OSC controller (`/pew/theme`).
- The HTTP control endpoint (`POST /api/v1/control` with `{ "theme": "lower-third" }`).
